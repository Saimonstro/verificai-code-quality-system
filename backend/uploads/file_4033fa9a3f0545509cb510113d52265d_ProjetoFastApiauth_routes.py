...
async def home():
    """
    Essa é a rota padrão de autenticação da nossa API
    """
    return {"message": "Essa é a rota de autenticação", "autenticado": False}
...
async def criar_conta(usuario_schema: UsuarioSchema, session: Session = Depends(pegar_sessao)):
    ...
    if usuario:
        ...
        session.commit()
        return {"message": f"Conta criada com sucesso email: {usuario_schema.email}"}
